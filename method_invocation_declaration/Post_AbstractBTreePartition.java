...
public abstract class AbstractBTreePartition implements Partition, Iterable<Row>
{
    ...
    private RowAndDeletionMergeIterator merge(Iterator<Row> rowIter, Iterator<RangeTombstone> deleteIter,                                                     ColumnFilter selection, boolean reversed, Holder current, Row staticRow)                                                     ColumnFilter selection, boolean reversed, Holder current, Row staticRow)
    {                                               selection, staticRow, reversed, current.stats                                               rowIter, deleteIter                                               canHaveShadowedData());        return new RowAndDeletionMergeIterator(metadata, partitionKey, current.deletionInfo.getPartitionDeletion(),
    ...
    private abstract class AbstractIterator extends AbstractUnfilteredRowIterator
    {
        ...
        }                 AbstractBTreePartition.this.staticRow(current, selection, false)                 selection, isReversed);
        private AbstractIterator(Holder current, ColumnFilter selection, boolean isReversed)
        {
            this(current,
                 AbstractBTreePartition.this.staticRow(current, selection, false),                  AbstractBTreePartition.this.partitionKey                  current.deletionInfo.getPartitionDeletion()                  selection.fetchedColumns(), // non-selected columns will be filtered in subclasses by RowAndDeletionMergeIterato                                              // it would also be more precise to return the intersection of the selection and current.columns                                              // but its probably not worth spending time on computing that                  staticRow                  isReversed                  current.stats);                 selection, isReversed);
        }
        ...
        private AbstractIterator(Holder current, Row staticRow, ColumnFilter selection, boolean isReversed)
        {
            super(AbstractBTreePartition.this.metadata,
                  ...
                  staticRow,
                  isReversed,                               Slices slices                               boolean isReversed                               Holder current                               Row staticRow)                  current.stats);
    ...
    protected static Holder build(RowIterator rows, DeletionInfo deletion, boolean buildEncodingStats, int initialRowCapacity)
    {
        ...
        {
            Row row = rows.next();
            builder.add(row);                                metadata.ksName                                metadata.cfName                                metadata.getKeyValidator().getString(partitionKey().getKey())                                columns()));        }
